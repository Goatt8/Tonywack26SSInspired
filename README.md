...
```
- Swift UI - 이미지 중심의 몰입형 Scroll View UI
- Firestore + Firebase Storage 기반 실시간 상품 데이터 관리
- 좌측 메뉴View 를 통한 카테고리 필터링
- 우측 서치View 를 통한 가격 및 검색 Range 필터링
...
* dimmed background( Black Opacity background )를 적용해 현재 활성화된 UI에 집중도 향상
```swift
